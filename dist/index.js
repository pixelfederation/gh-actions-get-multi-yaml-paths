...
const jsYaml = __importStar(__nccwpck_require__(917));
const jsonpath_plus_1 = __nccwpck_require__(697);
function yamlFileToJSONStr(file) {
    const yamlBody = fs.readFileSync(file, 'utf8');
    const jsonContent = jsYaml.load(yamlBody);
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        // extract inputs
        const yamlFile = core.getInput('yamlFile', { required: true });
        const pathsInput = core.getInput('paths', { required: true });
        const failUndefinedValue = core.getInput('failUndefinedValue').toLowerCase() === 'true' ? true : false;
        const failObjectValue = core.getInput('failObjectValue').toLowerCase() === 'true' ? true : false;
        try {
            // read file
            ...
            for (const path of paths) {
                ...
                if (typeof outputVarValue === 'undefined') {
                    ...
                    if (typeof outputVarValue === 'object') {
                        if (failObjectValue) {
                            core.setFailed(`Path ${keyPath} value is an Object and not primitive value.`);
                        }
                        else {
